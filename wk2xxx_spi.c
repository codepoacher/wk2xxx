...
static void wk2xxx_rx_chars(struct uart_port *port)//vk32xx_port *port)
{
	...
	{  
		...
		if (fsr& WK2XXX_RDAT)
		{
			...
			//printk(KERN_ALERT "rx_chars_wk2xxx_read_fifo!!!!!!!!!!!\n");
...
static int wk2xxx_startup(struct uart_port *port)//i
{
	...
	// request irq
	if(request_irq(s->port.irq, wk2xxx_irq,IRQF_SHARED|IRQF_TRIGGER_HIGH,"wk2xxxspi", (void*)s) < 0)
	{
...
static void wk2xxx_shutdown(struct uart_port *port)//
{
	...
	wk2xxx_read_reg(s->spi_wk,WK2XXX_GPORT,WK2XXX_GENA,dat);
	gena=dat[0];
	//printk(KERN_ALERT "%s iobase=%d  GENA = 0x%X\n",__func__,s->port.iobase,dat[0]);
	switch (s->port.iobase)
	{
		case 1:
			gena&=~WK2XXX_UT1EN;
...
static int wk2xxx_probe(struct spi_device *spi)
{
	...
	//printk(KERN_ALERT "wk2xxx_probe()  GENA1 = 0x%X\n",dat[0]);

	wk2xxx_write_reg(spi,WK2XXX_GPORT,WK2XXX_GENA,0xf5);
	wk2xxx_read_reg(spi,WK2XXX_GPORT,WK2XXX_GENA,dat);
	//printk(KERN_ALERT "wk2xxx_probe()  GENA2 = 0x%X\n",dat[0]);

	wk2xxx_write_reg(spi,WK2XXX_GPORT,WK2XXX_GENA,0xf0);
	wk2xxx_read_reg(spi,WK2XXX_GPORT,WK2XXX_GENA,dat);
	if((dat[0]&0xf0)!=0x30)
	{ 
	...
	for(i =0;i<NR_PORTS;i++)
	{
		...
		if(status<0)
		{
			...
			printk(KERN_ALERT "uart_add_one_port failed for line j:= %d with error %d\n",j,status);
		}
	}
...
static const struct of_device_id chip_spi_wk2xxx_dt_match[] = {
	{.compatible = "wkmic,wk2124spi"}, //和dts文件一致
	{},
};
MODULE_DEVICE_TABLE(of, chip_spi_wk2xxx_dt_match);
...
static struct spi_driver wk2xxx_driver = {
	.driver = {
		...
		.bus            = &spi_bus_type,
		.owner          = THIS_MODULE,
		.of_match_table = chip_spi_wk2xxx_dt_match,
	},
